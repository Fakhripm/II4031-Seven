...
export const GET = async (req: NextRequest, { params }: { params: { nim: string } } ) => {
    const nim = params.nim;

  if (!nim) {
    return NextResponse.json({message : "Missing nim parameter" }, { status: 400 });
  }
    try {
        const { data, error } = await supabase.from("akademik").select().eq("NIM", nim);
        if (error) {
            throw error;
        }
...
export const POST = async (req: NextRequest, { params }: { params: { nim: string } } ) => {
    const nim = params.nim;
